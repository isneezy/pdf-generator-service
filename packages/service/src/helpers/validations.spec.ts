...
describe('src/helpers/validation', () => {
  it('should validate goto parameter as url', () => {
    expect(() => PDFOptionsSchema.validateSync({ goto: 'hello.com'}))
      .to.throw('goto must be a valid URL')
  })

  it('should request template parameter when goto is not present', () => {
    expect(() => PDFOptionsSchema.validateSync({}))
      .to.throw('template is required when goto is not present')
  })
